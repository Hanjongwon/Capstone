...
from utils.general import (
    LOGGER, check_file, check_img_size, check_imshow, colorstr, cv2,
...
class CaptureIpCameraFramesWorker(QThread):
    ...
    def compute_hue_value(self, rgb_image, px_tmp, py_tmp):
        """
        주어진 RGB 이미지와 좌표(px_tmp, py_tmp)에서 7x7 영역의 RGB 평균 및 Hue 값을 계산합니다.
        """
        r = 0
        g = 0
        b = 0
        # RGB 평균을 계산할 7x7 영역
        for y in range(px_tmp + 1, px_tmp + 8, 1):
            for x in range(py_tmp + 1, py_tmp + 8, 1):
                color = rgb_image[x, y]
                r += color[0]
                g += color[1]
                b += color[2]
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            self.average_hue = self.compute_hue_value(rgbImage, px_tmp, py_tmp)
...
class MotorControl: #Motor_Control(Auto_Mode) Class
    ...
    def first_sequence(self):
        # check list에 Empty, Entering, Leaving 문자열이 5번 연속 삽입되었을 때, motor value -0.02(유체를 Chameber로 insert)
        if self.check_list == self.EMPTY_STATE or self.check_list == self.ENTERING_STATE or self.check_list == self.LEAVING_STATE:
    ...
    def second_sequence(self):
        ...
        if len(self.check_list) == self.STATE_COUNT:
            ...
            # check list에 Filled, Entering, Leaving 문자열이 5번 연속 삽입되었을 때, motor value -0.02(유체를 Chameber로 insert)
            if self.check_list == self.FILLED_STATE or self.check_list == self.ENTERING_STATE or self.check_list == self.LEAVING_STATE:
                if server1_on is True: #Platform1 motor_control
                    if servo1.value is not None:
                        print(f'server1_Motor_inhale')
                        servo1.value = servo1.value - 0.02
